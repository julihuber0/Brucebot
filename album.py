...
async def album_finder(ctx, *album):
    ...
    if checkDBforalbum(inputFixed):
        info = cur.execute(f"""SELECT album_name, album_year, song_url FROM ALBUMS WHERE LOWER(album_name) LIKE '{album.lower()}' ORDER BY song_num ASC""").fetchall()
        ...
        embed.add_field(name="Most Played:", value=f"{plays[-1][0]} ({plays[-1][1]}) / {plays[0][0]} ({plays[0][1]})", inline=False)
