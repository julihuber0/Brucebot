...
async def album_finder(ctx, *album):
    ...
    if album_info:
        ...
        embed.add_field(name="First/Last Premiered:", value=f"{premiere[0][0]} ([{main_url}{premiere[0][1]}]({first_date[0]})) / {premiere[-1][0]} ([{main_url}{premiere[-1][1]}]({last_date[0]}))", inline=False)
