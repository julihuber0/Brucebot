...
async def album_finder(ctx, *album):
    ...
    if album_info:
        ...
        plays = cur.execute(f"""select song_name, num_plays FROM SONGS WHERE song_url IN (SELECT song_url FROM ALBUMS WHERE LOWER(album_name) LIKE '%{album_to_find}%') ORDER BY CAST(num_plays AS integer) ASC""").fetchall()[0]
        ...
        embed.add_field(name="Most/Least Played:", value=f"{plays[0][0]} ({plays[0][1]}) / {plays[-1]} ({plays[-1][1]})", inline=False)
