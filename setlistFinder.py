    ...
    if cur.execute("""SELECT * FROM EVENTS WHERE event_date=?""", (date,)).fetchall():
      for r in cur.execute("""SELECT * FROM EVENTS WHERE event_date=?""", (date,)).fetchall():
        ...
        for s in cur.execute("""SELECT DISTINCT(set_type) FROM SETLISTS WHERE event_url=? ORDER BY setlist_song_id ASC""", (r[2],)).fetchall():
          setL = []
          key = ""
          temp = cur.execute("""SELECT song_name, song_url FROM SETLISTS WHERE event_url=? AND set_type=? ORDER BY song_num ASC""", (r[2], s[0],)).fetchall()
      
          for t in temp:
            song = t[0].replace("'", "''")
            premiere = cur.execute("""SELECT * FROM EVENTS WHERE setlist LIKE '%""" + song + "%' ORDER BY event_id ASC").fetchone()
            bustout = cur.execute("""SELECT * FROM EVENTS WHERE tour=? AND tour != '' AND setlist LIKE '%""" + song + "%' ORDER BY event_id ASC", (r[5],)).fetchone()
